-- Final Diagnostic for RLS Issue (Fixed)
...
-- 13. Create a completely new store and test (with proper UUID)
...
-- 14. Get the new store ID and test with it
WITH new_store AS (
    SELECT id FROM stores WHERE name = 'Test Store' AND owner_id = auth.uid() ORDER BY created_at DESC LIMIT 1
)
SELECT 
    'New store test' as info,
    COUNT(*) as accessible_records
FROM points_transactions pt, new_store ns
WHERE pt.store_id = ns.id;
